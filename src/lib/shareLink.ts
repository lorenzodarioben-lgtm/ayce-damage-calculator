...
import type { MealItem, MealSession, PlateSize, QualityTier } from '@/types/meal';
...
/**
 * Builds the token. Format, for version 1:
 *
 ...
 */
export function encodeSharePayload(session: MealSession): string | null {
  const items = session.items.filter((item) => FOOD_SHARE_CODES[item.foodId] !== undefined);
  if (items.length === 0) {
    return null;
  }
  ...
  const encodedItems = items
    ...
    .map((item) => {
      const code = FOOD_SHARE_CODES[item.foodId];
      const quality = QUALITY_ORDER.indexOf(item.quality);
      const plate = PLATE_ORDER.indexOf(item.plateSize);
      const quantity = Math.min(
        MAX_LINE_QUANTITY,
        Math.max(MIN_QUANTITY, Math.floor(item.quantity)),
      );
      return `${code}-${quality}-${plate}-${toBase36(quantity)}`;
    })
    .join('_');

  const price = toBase36(clampPricePerDiner(session.pricePerDiner) * 100);
  const diners = toBase36(clampDinerCount(session.dinerCount));
  const name = encodeText(sanitiseRestaurantName(session.restaurantName).trim());

  return `${SHARE_TOKEN_VERSION}.${price}.${diners}.${encodedItems}.${name}`;
}
...
export function decodeSharePayload(token: string | null | undefined): SharePayload | null {
  ...
  if (Number(versionRaw) !== SHARE_TOKEN_VERSION || !/^\d+$/.test(versionRaw)) {
    return null;
  }
...
export function shareLinkPath(session: MealSession): string | null {
