    ...
    <li className="panel lift-on-hover relative p-4 hover:border-line-ember hover:elevate-raised">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="min-w-0">
          <p className="micro-label text-cream-500">{formatRecordedAt(record.createdAt)}</p>
          {/* The whole card is the link target; the delete button sits above it. */}
          <h3 className="mt-1 break-words text-ui font-bold text-cream-50">
            <Link href={`/history/${record.id}`} className="after:absolute after:inset-0">
              {label}
            </Link>
          </h3>
          <p className={cn('display-type mt-1.5 text-lead', TONE[verdict.tone])}>{verdict.title}</p>
          ...
          {record.note && (
            <p className="mt-1.5 line-clamp-2 break-words text-caption leading-snug text-cream-500">
              {record.note}
            </p>
          )}
      ...
      <dl className="tabular mt-4 grid grid-cols-2 gap-x-3 gap-y-3 border-t border-line-soft pt-3 sm:grid-cols-4">
        ...
        <div>
          <dt className="micro-label text-cream-500">Retail</dt>
          <dd className="text-body font-bold text-cream-50">
        ...
        <div>
          <dt className="micro-label text-cream-500">Volume</dt>
          <dd className="text-body font-bold text-cream-50">
